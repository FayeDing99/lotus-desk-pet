...
可复用的 OC 桌宠原型。使用透明 PNG，包含轻柔待机动画、窗口/预览拖动、右键跟随轮盘、戳一戳互动、可选大模型对话、分场景本地话本、天气内容、久坐提醒、状态与可自定义荷包，以及本地持久化。
...
右键角色打开“话本”，可上传或拖入单张 PNG/WebP，也可选择一个 OC 分层动画文件夹。大小、位置、名字、完整人设、自称、口头禅、分场景话本、主动发言间隔和久坐提醒都可以独立调整。配置和图片保存在本地；“导出”会生成包含形象、动画包和话本的 JSON 配置包，其他 OC 可以直接“导入”复用。

### 大模型与实时内容

在“话本”底部展开“大模型与实时内容”，可以选择 OpenAI、Claude、DeepSeek 或 OpenAI Chat Completions 兼容接口，自定义模型名和接口地址。开启后：

- 主动聊天优先使用大模型，并把完整 OC 人设、自称、称呼、说话范例、当前状态和天气与用户消息一起生成回答。
...
本项目实现的是轻量、自有格式的 OC 二维动画包，不依赖 Live2D Cubism SDK，也不需要为每次播放动作付费。可在“话本 → 导入 OC 动画包”中直接选择分层 PSD，也可选择由同画布透明 PNG/WebP 图层组成的整个文件夹。PSD 只在本地解析，不会上传到网络。基础运行时会自动提供：
...
PSD 导入会保留可见图层的位置、顺序和透明度，隐藏图层不会导入。程序会结合图层名与分组名自动识别 `body/身体`、`head/头`、`eyes/眼`、`mouth/嘴`、`hair-back/后发`、`hair-front/前发`和 `accessory/饰品`；未识别的图层会作为静态图层保留。它不是骨骼变形系统：当前主要是图层级的缩放、旋转、位移和眨眼；头发网格变形、大幅转头和精准口型仍需后续的骨骼/网格运行时。

## 编辑基础本地话库

默认分场景台词位于 `public/dialogue.json`。桌宠的“话本 · TA 可能说的话”中可以逐类覆盖；每行一句，留空时使用内置话本。未启用大模型时，聊天使用 `fallback` 台词，不再做关键词匹配。

## 本地数据

状态与 OC 配置保存在 WebView/浏览器的 `localStorage` 中，上传的角色图片保存在 IndexedDB 中。饱腹度每 15 分钟下降 1，精力每 20 分钟下降 1；低状态会缓慢影响心情。荷包中的点心和道具可分别自定义名称、属性变化与使用台词，并可无限次使用。

## GitHub 发布与自动更新

完整步骤图、首次配置、脱敏检查和发布失败处理见 [GitHub 发布与应用更新流程](docs/UPDATE_WORKFLOW.md)。
...
发现新版本时，桌面端会自动弹出更新卡片，显示版本号、Release 更新说明和下载进度。用户可以立即安装、延后 24 小时提醒、跳过当前版本，或选择以后自动下载安装；这些偏好只保存在本机。浏览器开发预览可使用 `http://127.0.0.1:1420/?update-preview=1` 检查弹窗样式，但不会真正安装更新。
